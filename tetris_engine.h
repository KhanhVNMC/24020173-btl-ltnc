...
public:
	...
	int lockDelay;
	// hold toggler, different from the HOLD flag that the context uses
...
inline void TetrisEngine::onEngineStart() {
	//this.pushNextPieceToPlayfield(); // TODO
}
...
inline void TetrisEngine::onUserHold() {
	if (!holdEnabled || !canHold) return; // Exit if holding is not allowed or disabled altogether
	...
	// Place the held piece back into the playfield if it exists
	if (this->holdPiece != nullptr) {
		//this->putPieceInPlayfield(this.holdPiece);
	} else {
		//this->pushNextPieceToPlayfield(); // Place the next piece in playfield
	}

	this->holdPiece = toHold; // Update holdPiece with the current falling piece
...
inline void TetrisEngine::updatePlayfieldState(Tetromino* locked) {
	// flags for the event
		...
		if (clearedLines.size() > 0) {
			...
			//if (comboCount > 0 && onCombo != null) onCombo.accept(comboCount);
		} else if (comboCount > 0) {
			//if (onComboBreaks != null) onComboBreaks.accept(comboCount);
			comboCount = -1; // broke
		}

		// fire the event for user
		//if (this->onMinoLocked != null) onMinoLocked.accept(clearedLines.size());

		// the playfield event emitter
		/*
		if (this->onPlayfieldEvent != null && (isMiniSpin || isSpin || perfectClear || clearedLines.size() > 0)) {
			// fire the event
			this.onPlayfieldEvent.accept(
				new PlayfieldEvent(
					clearedLines.stream().mapToInt(i -> i).toArray(),
					perfectClear,
					locked->type, isSpin, isMiniSpin
				)
			);
		}
		*/
	...
	if (this->started) throw invalid_argument("This instance is already started");
		...
		for (;;) {
			...
			*/
			...
			if (shouldTopOut) {
				//if (this.onTopOut != null) this.onTopOut.run(); // execute one last time
